...
- 經過我們的測試，我們發現「點回歸模型」雖然能達到更好的評價分數，但在實際應用中，熱圖模型的效果更穩定，因此我們仍會推薦您使用熱圖模型。

- 我們仍無法割捨「點回歸模型」的優點，包含但不限於：可以預測圖面範圍之外的角點；快速簡單的後處理流程等。因此我們會持續優化「點回歸模型」，以提升其效果。

- HeatmapRec-LC100-256 模型相關數據：
    - 參數量：約為 1.2 百萬
    - FP32 模型大小：約為 4.9 MB
    - 運算量：約為 1.6 FLOPs(G)。
...
- **Backbone: LCNet-100**
    ...
    在這個模型中，使用的是 LCNet-100，這是一種輕量級的卷積神經網絡，特別適用於在計算資源受限的環境下進行高效的特徵提取。我們預期 LCNet-100 應該能從輸入數據中提取出足夠的特徵信息，為後續的熱圖回歸做好準備。

- **Neck: BiFPN**

    Neck 部分是用於增強從 Backbone 流出的特徵的。
