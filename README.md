...
- [快速開始](#快速開始)
    ...
    - [結果處理](#結果處理)
        ...
        - [提取攤平後的文件圖像](#提取攤平後的文件圖像)
        - [將文件資訊轉為 JSON](#將文件資訊轉為-json)
        - [範例](#完整範例)
- [Benchmark](#benchmark)
- [訓練模型](#訓練模型)
    - [資料集介紹](#資料集介紹)
...
- 經過我們的測試，我們發現點檢測的模型雖然能達到更好的 benchmark 分數，但在實際應用中，熱圖模型的效果更穩定，因此我們仍會推薦您使用熱圖模型。
...
在開始說明前，我們也體諒到您可能只有經費，而沒有足夠的時間對您的現場環境進行客製化調整。

因此您也可以直接聯絡我們進行諮詢，我們可以根據您施工難度，幫您調度工程師來進行客製化的開發。
...
另外，如果您不急迫，您可以直接把您的文本或場景資料集提供給我們，我們會在未來的版本中，將您的資料集納入我們的測試資料集中，並且在未來的版本中，提供更好的模型效果。這種方式對您來說是完全免費的。
...
- **CoarseDropout**
   - 這個增強技術會隨機在圖像中產生一個矩形區域，並將該區域內的像素值設為隨機顏色。可以模擬圖像中的遮擋，例如圖像中的文字被其他物體遮擋的情況。
